...
class ControllerExtensionTest < Minitest::Test
  ...
  def test_the_delegate_is_added
    assert_equal Authie::ControllerDelegate, @controller.send(:stream_session_delegate).class
  end

  def test_before_actions_are_added
    assert @controller.class.before_actions.include?(:set_browser_id)
    assert @controller.class.before_actions.include?(:touch_stream_session)
  end

  def test_helper_methods_are_added
    assert @controller.class.helper_methods.include?(:stream_in?)
    assert @controller.class.helper_methods.include?(:current_stream)
    assert @controller.class.helper_methods.include?(:stream_session)
  end

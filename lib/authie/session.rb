...
module Authie
  class Session < ActiveRecord::Base
    ...
    scope :for_user, -> (user) { where(:order_type => user.class.name, :order_id => user.id) }
    ...
    def user
      if self.order_id && self.order_type
        @user ||= self.order_type.constantize.find_by(:id => self.order_id) || :none
        @user == :none ? nil : @user
      end
    end
    ...
    def user=(user)
      if user
        self.order_type = user.class.name
        self.order_id = user.id
      else
        self.order_type = nil
        self.order_id = nil
      end
